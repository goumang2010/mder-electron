  ...
  child.stdout.on('data', data => {
    ...
    if (!isWeb) {
      if (/VALID/g.test(data.toString().trim().replace(/\n/g, '\n' + repeat(' ', command.length + 2))) && !isElectronOpen) {
        ...
        run('cross-env NODE_ENV=development electron app/main.js --debug', BLUE, 'electron')
        isElectronOpen = true
      }
    }
  })
...
run(`${isWeb ? `start http://localhost:${config.port} & ` : ''} webpack-dev-server --inline --hot --colors`, YELLOW, 'webpack')
